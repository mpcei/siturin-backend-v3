...
@Injectable()
export class ProcessFoodDrinkService {
  constructor(
  ...
  async createRegistration(
    payload: CreateRegistrationProcessFoodDrinkDto,
    user: UserEntity,
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      await this.saveProcessFoodDrink(payload, manager);
  ...
  private async saveProcess(
    ...
    process.localTypeId = payload.localType.id;
    process.inspectionExpirationAt = addDays(endOfDay(new Date()), 114);

    return await processRepository.save(process);
  }

  private async saveProcessFoodDrink(
    payload: CreateRegistrationProcessFoodDrinkDto,
    manager: EntityManager,
  ): Promise<ProcessFoodDrinkEntity> {
    const processFoodDrinkRepository = manager.getRepository(ProcessFoodDrinkEntity);

    let processFoodDrink = await processFoodDrinkRepository.findOneBy({
      processId: payload.processId,
    });

    if (!processFoodDrink) {
      processFoodDrink = processFoodDrinkRepository.create();
    }
    ...
    return await processFoodDrinkRepository.save(processFoodDrink);
  }
